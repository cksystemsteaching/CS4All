    ...
    if (symbol == SYM_ASTERISK) {
        ...
        if (isIncrementOrDecrement()) {
            ...
            if (symbol == SYM_IDENTIFIER) {
                ...
                //increase or decrease pointer (that is, add or sub WORDSIZE)
                emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(), incrDecrValue);
                //store new value back to the pointer's address in memory
                emitIFormat(OP_SW, getScope(entry), currentTemporary(), getAddress(entry));
            ...
            if (symbol == SYM_IDENTIFIER) {
                ...
                if (symbol == SYM_LPARENTHESIS)
                    ...
                    if (entry == (int *) 0) {
                        ...
                        createSymbolTableEntry(GLOBAL_TABLE, variableOrProcedureName, currentLineNumber, VARIABLE, type,
                                               initialValue, -allocatedMemory);
                    } else {
...
int main(int argc, int *argv) {
    ...
    print((int *) "This is easySE Selfie");
    println();
